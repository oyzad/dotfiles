...
apps_mgoff_normal="(iloader|VirtualBox)"
...
if [[ $(echo "$display_resolution" | grep -c "Resolution") -ge 2 ]]; then
  ...
  # apps
  apps_mgoff_below="(Calculator|Cog|VLC|GIMP|Notes|Activity Monitor|App StoreSoftware Update|Brave|Google Chrome|Stickies|kitty|Photo Booth|Spotify|Finder|System Preferences)"
else
  apps_stream="()"
  # This keeps apps always below, seems to be working fine when I switch to other
  # apps
